...
public class GyroPIDController extends PIDController {
    ...
    public GyroPIDController(IBNO055IMU gyro, double threshold, double slowDownStart, double fineTuneStart, DcMotor... motors) {
    ...
    public void setTargets(double target) {
        ...
        double absoluteAngle = currentPosition + target;
        targets[0] =  (currentPosition < 0 ? absoluteAngle - 360 : absoluteAngle);
        if (absoluteAngle > 180)
            targets[0] = (absoluteAngle - 360);
        else if (absoluteAngle < -180)
            targets[0] = (360 - absoluteAngle);

        //targets[0] = (currentPosition + target) % 360;

        sign = targets[0] - currentPosition > 0 ? 1 : -1;
    }
    ...
    public double[] run() {
        ...
        if (!hasReachedDestination()) {
            ...
            /*if (Math.abs(error) > 180) {
                error = currVal - (targets[0] + 360);
            }*/

            if (Math.abs(error) >= threshold) {
                double powerMultiplier = (Math.abs(error) >= fineTuneStart) ? powerDescentFast : powerDescentSlow;
                power[0] = Math.abs(error) * powerMultiplier;
            }
            power[0] = sign * Range.clip(power[0], -powerMax, powerMax);
